...
scope: sf-repurpose-repurpose-workflow
owner: unknown
confidence: high
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - skills/sf-repurpose/references/repurpose-workflow.md
depends_on:
...
supersedes: []
evidence:
  - "Extracted from skills/sf-repurpose/SKILL.md during Compact ShipFlow Skill Instructions Phase 2."
...
This reference preserves the detailed pre-compaction instructions for `sf-repurpose`. The top-level `SKILL.md` is now the activation contract; load this file only when the selected mode needs the detailed workflow, checklist, templates, or examples below.
