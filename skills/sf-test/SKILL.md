...
- write or update a durable bug file under `bugs/*.md` when the result fails
- refresh optional `BUGS.md` triage only when present or generated by the project workflow
...
- `--retest BUG-ID`: retest a known bug from `bugs/BUG-ID.md`, using optional `BUGS.md` only as triage context
...
Rules:
- `TEST_LOG.md` stays compact. It is a tracker, not a full bug file.
...
## Step 6 — Professional Bug Model (Bug File + Optional Index)

When a scenario fails (or `--retest BUG-ID` is used), follow the bug-file-first model:
- `TEST_LOG.md`: compact scenario history and pointers
- `bugs/BUG-ID.md`: detailed Markdown source of truth for one bug work item (`artifact: bug_record`)
- `BUGS.md`: optional compact/generated triage index when present or generated by the project workflow
...
- no other transition is allowed transition unless explicitly justified in the bug file
...
## Step 7 — Write or Update bugs/BUG-ID.md and Optional BUGS.md

If the normalized result is `fail`, or when running `--retest BUG-ID`, update `bugs/BUG-ID.md`. Update `BUGS.md` only when that optional triage view exists or is generated by the project workflow.

### 7.1 Compact BUGS.md index rules

Right before editing `BUGS.md`, re-read it from disk. If it is absent, do not create it unless the project workflow generates the optional triage view.

`BUGS.md`, when present, must stay compact. Each entry is a pointer, not a full narrative.

Recommended index line format:

```markdown
- BUG-YYYY-MM-DD-001 | open | high | [short title] | last-tested: YYYY-MM-DD | bug-file: bugs/BUG-YYYY-MM-DD-001.md
```
...
1. List files `bugs/BUG-YYYY-MM-DD-*.md` and collect suffixes.
2. Re-read optional `BUGS.md` if present and collect same-day suffixes from lines containing `BUG-YYYY-MM-DD-*`.
3. Pick `NNN` greater than every suffix found in either place.

Immediately before writing the bug file:
1. Check whether `bugs/BUG-ID.md` already exists.
2. If it exists, re-read `bugs/` and optional `BUGS.md` once, increment suffix, and retry.
3. If collision repeats after retry, stop and report collision; do not overwrite.

### 7.3 Duplicate detection

Before creating a new bug file, search for an open bug with clearly matching reproduction + observed behavior.
...
### 7.4 Bug file update rules (`bugs/BUG-ID.md`)
...
`--retest BUG-ID` is bug-file-first:
1. Read `bugs/BUG-ID.md` and optional `BUGS.md` if present.
2. Re-run reproduction steps from the bug file.
3. Append one line to bug-file `Retest History`.
4. Update bug-file status with allowed transitions:
...
Files updated:
- TEST_LOG.md (compact)
- bugs/BUG-ID.md (bug file) [if applicable]
- BUGS.md (optional compact triage index) [if applicable]
...
- Record operational evidence in `TEST_LOG.md`, keep full bug context in each bug file, and keep `BUGS.md` as an optional compact triage index.
- Do not put YAML frontmatter in `TEST_LOG.md` or `BUGS.md`.
- Keep bug file entries actionable enough for `sf-fix`.
