...
scope: sf-audit-audit-master-workflow
owner: unknown
confidence: high
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - skills/sf-audit/references/audit-master-workflow.md
depends_on:
...
supersedes: []
evidence:
  - "Extracted from skills/sf-audit/SKILL.md during Compact ShipFlow Skill Instructions Phase 3."
...
This reference preserves the detailed pre-compaction instructions for `sf-audit`. The top-level `SKILL.md` is now the activation contract; load this file only when the selected mode needs the detailed workflow, checklist, templates, provider routing, examples, or report details below.
