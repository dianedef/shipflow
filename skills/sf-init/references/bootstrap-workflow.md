...
scope: sf-init-bootstrap-workflow
owner: unknown
confidence: high
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - skills/sf-init/references/bootstrap-workflow.md
depends_on:
...
supersedes: []
evidence:
  - "Extracted from skills/sf-init/SKILL.md during Compact ShipFlow Skill Instructions Phase 2."
...
This reference preserves the detailed pre-compaction instructions for `sf-init`. The top-level `SKILL.md` is now the activation contract; load this file only when the selected mode needs the detailed workflow, checklist, templates, or examples below.
