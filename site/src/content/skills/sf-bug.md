...
title: "sf-bug"
slug: "sf-bug"
tagline: "Route a bug from report to dossier, fix, retest, verification, and ship risk without skipping gates."
summary: "A professional bug loop orchestrator that reads bug state and sends the operator to the right next ShipFlow skill."
category: "Build & Fix"
audience:
  - "Founders managing bugs across multiple agent sessions"
  - "Developers who want one command to route bug state safely"
  - "Operators using BUG-ID dossiers as durable project memory"
problem: "ShipFlow already has bug capture, fix, retest, verification, and shipping gates, but the operator still has to remember which command comes next for each bug state."
outcome: "You get the next correct bug command based on the dossier status, severity, evidence needs, development mode, and ship risk."
...
when_to_use:
  ...
  - "When a fresh bug report needs routing to test, fix, browser evidence, or spec"
what_you_give:
...
argument_modes:
  - argument: "no argument"
    ...
    consequence: "Recommends the highest-priority next command instead of editing records."
  - argument: "BUG-ID"
    effect: "Reads BUGS.md and bugs/BUG-ID.md, then routes based on status and severity."
    consequence: "Keeps the dossier as the source of truth."
  - argument: "--retest BUG-ID"
    effect: "Routes to sf-test --retest BUG-ID."
...
```text
What should happen next for this bug?
```

It reads the bug state and routes to the owner skill. It does not patch code, mutate the dossier, or declare closure by itself.
...
That makes the loop easier to run without weakening it. A `fix-attempted` bug routes to retest. A `fixed-pending-verify` bug routes to verification. An open high or critical bug blocks clean ship framing.
...
`sf-bug` makes those states explicit and gives the next command. It preserves the professional bug loop while keeping each specialized skill responsible for its own work.
